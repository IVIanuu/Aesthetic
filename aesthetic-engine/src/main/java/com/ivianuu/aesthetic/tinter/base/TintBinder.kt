...
package com.ivianuu.aesthetic.tinter.base
...
internal class TintBinder : AestheticInflationFactory.Interceptor {
    ...
    override fun onViewInflated(view: View, attrs: AttributeSet, parent: View?) {
        ...
        val tinter: Tinter? = when (view) {
            ...
            is ImageView -> {
                when (view) {
                    is FloatingActionButton -> FloatingActionButtonTinter(
                        view,
                        attrs
                    )
                    is ImageButton -> ImageButtonTinter(
                        view,
                        attrs
                    )
            ...
            is NestedScrollView -> NestedScrollViewTinter(
                view,
                attrs
            )
            is ProgressBar -> {
            ...
            is SwipeRefreshLayout -> SwipeRefreshLayoutTinter(
                view,
                attrs
            )
            is TabLayout -> TabLayoutTinter(view, attrs)
            is TextInputLayout -> TextInputLayoutTinter(
                view,
                attrs
            )
            is TextView -> {
                when (view) {
                    is ActionMenuItemView -> ActionMenuItemViewTinter(
                        view,
                        attrs
                    )
                    is Button -> {
                        when (view) {
                            is CheckBox -> CheckBoxTinter(
                                view,
                                attrs
                            )
                            is RadioButton -> RadioButtonTinter(
                                view,
                                attrs
                            )
                            is Switch -> SwitchTinter(view, attrs)
                            is SwitchCompat -> SwitchCompatTinter(
                                view,
                                attrs
                            )
                            else -> {
                    ...
                    is EditText -> {
                        if (view is TextInputEditText) {
                            TextInputEditTextTinter(view, attrs)
                        } else {
                            EditTextTinter(view, attrs)
                        }
                    }
                    else -> {
